...
static void http_server_fw_update_reset_timer(void)
{
	if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL)
	{
		...
		// Give the web page a chance to receive an acknowledge back and initialise the timer
		...
		if (xQueueReceive(http_server_monitor_queue_handle, &msg, portMAX_DELAY))
		{
			switch (msg.msgID)
			{
				...
				case HTTP_MSG_OTA_UPDATE_FAILED:
					...
					g_fw_update_status = HTTP_MSG_OTA_UPDATE_FAILED;
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		if (!is_req_body_started)
		{
			...
			{
				printf("http_server_OTA_update_handler: Writing to partition sub type %d at offset 0x\r\n", update_partition->subtype);
			}
	...
	if (esp_ota_end(ota_handle) == ESP_OK)
	{
		...
		if (esp_ota_set_boot_partition(update_partition) == ESP_OK)
		{
			...
			ESP_LOGI(TAG, "http_server_OTA_update_handler: Next boot partition subtype %d at offset 0x", boot_partition->subtype);
...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
{
	...
	sprintf(dhtSensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}", rand()%(30-25)+25.0, rand()%(80-70)+70.0);
